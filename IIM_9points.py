...
def irregular_points_coefficient_minus(x,y,dx,dy):
    ...
    for k in k_plus:
        a_matrix[1,k]=rou*stencil_local[k][0]
    for k in k_plus:
        a_matrix[1,k]=a_matrix[1,k]-0.5*stencil_local[k][0]**2*(1-rou)*kappa
    for k in k_plus:
        a_matrix[1,k]=a_matrix[1,k]+0.5*stencil_local[k][1]**2*(1-rou)*kappa
    #b[1]=0
    ...
    for k in k_plus:
        a_matrix[2,k]=stencil_local[k][1]
    for k in k_plus:
        a_matrix[2,k]=a_matrix[2,k]+stencil_local[k][0]*stencil_local[k][1]*(1-rou)*kappa
    #b[2]=0

    for k in k_minus:
        a_matrix[3,k]=0.5*stencil_local[k][0]**2
    for k in k_plus:
        a_matrix[3,k]=0.5*stencil_local[k][0]**2*rou
    ...
    for k in k_plus:
        a_matrix[4,k]=0.5*stencil_local[k][1]**2
    for k in k_plus:
        a_matrix[4,k]=a_matrix[4,k]+0.5*stencil_local[k][0]**2*(rou-1)
    b_vector[4]=beta_minus

    for k in k_minus:
        a_matrix[5,k]=stencil_local[k][0]*stencil_local[k][1]
    for k in k_plus:
        a_matrix[5,k]=stencil_local[k][0]*stencil_local[k][1]*rou
    #b[5]=0
    #求解待定系数
    g=np.zeros(9)#逼近系数g
...
def irregular_points_coefficient_plus(x, y, dx, dy):
    ...
    for k in k_minus:
        a_matrix[1, k] = stencil_local[k][0]/rou
    for k in k_plus:
        a_matrix[1, k] = stencil_local[k][0]
    for k in k_minus:
        a_matrix[1, k] = a_matrix[1, k] + 0.5 * stencil_local[k][0] ** 2 * (1/rou-1) * kappa
    for k in k_minus:
        a_matrix[1, k] = a_matrix[1, k] - 0.5 * stencil_local[k][1] ** 2 * (1/rou-1) * kappa
    # b[1]=0
    ...
    for k in k_plus:
        a_matrix[2, k] = stencil_local[k][1]
    for k in k_minus:
        a_matrix[2, k] = a_matrix[2, k] + stencil_local[k][0] * stencil_local[k][1] * (1 - 1/rou) * kappa
    # b[2]=0
    ...
    for k in k_plus:
        a_matrix[4, k] = 0.5 * stencil_local[k][1] ** 2
    for k in k_minus:
        a_matrix[4, k] = a_matrix[4, k] + 0.5 * stencil_local[k][0] ** 2 * (1/rou - 1)
    ...
    for j in range(N-2):
        ...
        if is_regular(x_point,y_point,dx,dy):
            ...
            if i == 0 and j == 0:
                ...
                beta_point * fun_analysis(x[0],y[1]) / dx ** 2-beta_point*fun_analysis(x[1],y[0])/dy**2
            elif i == 0 and j == N - 3:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) -\
                beta_point * fun_analysis(x[0],y[N-2]) / dx ** 2-beta_point*fun_analysis(x[1],y[N-1])/dy**2
            elif i == N - 3 and j == 0:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) -\
                beta_point * fun_analysis(x[N-1],y[1]) / dx ** 2-beta_point*fun_analysis(x[N-2],y[0])/dy**2
            elif i == N - 3 and j == N - 3:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) -\
                beta_point *fun_analysis(x[N-1],y[N-2]) / dx ** 2-beta_point*fun_analysis(x[N-2],y[N-1])/dy**2

            elif i == 0:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) - beta_point * fun_analysis(x[0],y_point) / dx ** 2
            elif i == N - 3:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) - beta_point * fun_analysis(x[N-1],y_point) / dx ** 2
            elif j == 0 :
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) - beta_point * fun_analysis(x_point,y[0]) / dy ** 2
            elif j == N - 3:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) - beta_point * fun_analysis(x_point,y[N-1]) / dy ** 2
            else:
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point)
        else:
            if which_side_of_interface(x_point, y_point) ==-1:  # 在-侧或界面上
                coefficient_minus, C_minus, stencil_index_minus = irregular_points_coefficient_minus(x_point, y_point, dx, dy)
                for k_stencil, (i_stencil, j_stencil) in enumerate(stencil_index_minus):
                    a_matrix[i * (N - 2) + j, (i + i_stencil) * (N - 2) + j + j_stencil] = coefficient_minus[k_stencil]
                b_vector[i * (N - 2) + j] = fun_source(x_point, y_point) + C_minus
            else:  # 在+侧
                coefficient_plus, C_plus, stencil_index_plus = irregular_points_coefficient_plus(x_point, y_point, dx, dy)
                for k_stencil, (i_stencil, j_stencil) in enumerate(stencil_index_plus):
                    a_matrix[i * (N - 2) + j, (i + i_stencil) * (N - 2) + j + j_stencil] = coefficient_plus[k_stencil]
